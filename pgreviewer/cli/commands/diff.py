...
def run_diff(diff_file: Path, json_output: bool, only_critical: bool) -> None:
    ...
    for cf in changed_files:
        path_str = cf.path
        local_path = (
            diff_file.parent.joinpath(path_str)
            if not diff_file.is_absolute()
            else Path(path_str)
        )
        # Assuming run from repo root, it is safer to use Path(path_str)
