...
class TestPreCheckUnderBudget:
    def test_under_budget_does_not_raise(self, cost_dir: Path) -> None:
        ...
        # $0.001 well under $5.00 limit for review (50% of $10)
        g.pre_check("review", estimated_cost_usd=0.001)

    def test_zero_cost_passes(self, cost_dir: Path) -> None:
        g = _make_guardrail(cost_dir)
        g.pre_check("review", estimated_cost_usd=0.0)
...
class TestPreCheckAtLimit:
    def test_exceeding_budget_raises(self, cost_dir: Path) -> None:
        g = _make_guardrail(cost_dir, monthly_budget=1.0)
        # review limit = 1.0 * 0.5 = $0.50; record $0.49 spent
        g.record("review", 0.49)
        # $0.02 → 0.49 + 0.02 = 0.51 > 0.50
        with pytest.raises(BudgetExceededError):
            g.pre_check("review", estimated_cost_usd=0.02)

    def test_exactly_at_limit_passes(self, cost_dir: Path) -> None:
        g = _make_guardrail(cost_dir, monthly_budget=1.0)
        g.record("review", 0.49)
        # $0.01 → 0.49 + 0.01 = 0.50 == limit → passes
        g.pre_check("review", estimated_cost_usd=0.01)

    def test_unknown_category_has_zero_limit(self, cost_dir: Path) -> None:
        g = _make_guardrail(cost_dir, monthly_budget=1.0)
        with pytest.raises(BudgetExceededError):
            g.pre_check("nonexistent", estimated_cost_usd=0.01)
